...
export const Navbar: FC<NavbarProps> = ({ openSearch, openSetup, ...props }) => {
		...
		getCoords().then(c => {
			getForecastDataByCoords().then(data => {
				if (!data.error) {
					...
					const query = coordsQuery || resolveQueryFromData(data)
	...
	return (
		<Flex align='center' as='nav' gap={2} justify='flex-start' p={2} {...props}>
			{fetching && (
				<ToolTip fontSize='md' id='spinner-icon' tooltipLabel={t('tooltips.fetching')}>
					<Flex px={2}>
			...
			<ToolTip fontSize='md' id='search-icon' tooltipLabel={t('tooltips.search')}>
				<NavIcon aria-label='search icon' icon={<SearchIcon />} onClick={openSearch} />
			</ToolTip>

			<ToolTip fontSize='md' id='geolocation-icon' tooltipLabel={t('tooltips.geolocation')}>
				<NavIcon aria-label='geolocation icon' icon={<GeoIcon />} onClick={getGeolocationForecast} />
			</ToolTip>

			<ToolTip fontSize='md' id='cog-icon' tooltipLabel={t('tooltips.setup')}>
