...
describe.concurrent('GET /api/forecast', () => {
	...
	describe.concurrent('with a valid query', () => {
		...
		describe.concurrent('body.location', () => {
			it('has expected properties', async () => {
				...
				const { location: o } = body as ForecastResponse
		...
		describe.concurrent('body.currentWeather', () => {
			it('has expected properties', async () => {
				const { body } = await api.get(VALID_URL)
				const { currentWeather: o } = body as ForecastResponse
		...
		describe.concurrent('body.forecast', () => {
			it(`is an array with ${forecastDays} childs`, async () => {
				const { body } = await api.get(VALID_URL)
				const { forecast: data } = body as ForecastResponse
			...
			describe.concurrent('body.forecast[0]', () => {
				it('has expected properties', async () => {
					const { body } = await api.get(VALID_URL)
					const { forecast: data } = body as ForecastResponse
					const o = data[0]
				...
				describe.concurrent('body.forecast[0].day', () => {
					it('has expected properties', async () => {
						const { body } = await api.get(VALID_URL)
						const { forecast: data } = body as ForecastResponse
						const o = data[0].day
				...
				describe.concurrent('body.forecast[0].astro', () => {
					it('has expected properties', async () => {
						const { body } = await api.get(VALID_URL)
						const { forecast: data } = body as ForecastResponse
				...
				describe.concurrent('body.forecast[0].hours', () => {
